...
export default function HalamanSurah({ noSurat }) {
    ...
    useEffect(() => {
        ...
        fetchData();
    }, [noSurat]); // Include noSurat in the dependency array to fetch data when it changes
    ...
    // console.log(datas.data.data[1]);
    return (
        <>
            {datas ? (
                <div className="w-full">
                    <span className="flex justify-between mb-5 m-4 bg-slate-300 rounded-lg p-4 font-semibold text-slate-700 drop-shadow-md">
                        <p className="bg-slate-50 rounded-md p-1 drop-shadow-md">
                            {datas.data.data[0].surah.location}
                        </p>
                        <p className="bg-slate-50 rounded-md p-1 drop-shadow-md">
                            {datas.data.data[0].surah.translation}
                        </p>
                        <p className="bg-slate-50 rounded-md p-1 drop-shadow-md">
                            {datas.data.data[0].surah.num_ayah} Ayat
                        </p>
                    </span>
                    <span className="overflow-y-auto">
                        {datas.data.data.map((dat, i) => (
                            <div
                                key={i}
                                className={`gap-2 px-2 py-1 flex ${
                                    ...
                                    <span className="flex flex-col justify-end ">
                                        <span className="flex justify-end">
                                            <p
                                                ...
                                                className="text-3xl mb-4 text-end"
                                            >
                                                {dat.kitabah}
                                            </p>
                                        </span>
                                        ...
                                        <p className="mb-5">
                                            {removeNumbering(dat.translation)}
                                        </p>
                                    </span>
                                </span>
                            </div>
